...
public class SortedGraph {
	 ...
	 * @return the rootNode
	 */
	public static GraphNode importFile(String ifile, String rootcaption, Monitor mon) {
		...
		try (BufferedReader br = new BufferedReader(new FileReader(ifile))) {
			...
			mon.postErrorMessage("IOException. Abort.");
			e.printStackTrace();
			return null;
		}
		...
		if (graphNeedsUpdateOnLeafSizes) {
			mon.postUrgentMessage("subtreesize needs update.");
			root.updateTreeSize();
			mon.postUrgentMessage("subtreesize updated.");
		} else {
			mon.postUrgentMessage(
					"subtreesize is already specified in the file. If you don't want it, erase at least one entry.");
		}
		if (graphNeedsPlot) {
			mon.postUrgentMessage("Tree needs plot.");
		} else {
			mon.postUrgentMessage("It appears, that the graph is already plotted. If you want to "
					+ "force plot it, change the radius of root to something different than 1.0.");
		}
		mon.postUrgentMessage("Import of " + rootcaption + " from " + ifile + " completed.");
		return root;
	}
